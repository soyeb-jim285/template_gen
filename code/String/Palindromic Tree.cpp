...
/***
    * str is 1 based
    ...
    It's a directed tree. If we reverse the direction of the suffix links, we
get a dag. In this DAG, if node v is reachable from node u iff, u is a substring
of v.
    * if ( tree[A].next[x] == B )
      then, B = xAx
    ...
    * st[i] denotes starting index of the first occurrence of the corresponding
palindrome
...
int nc, suff, pos;
/// nc -> node count
/// suff -> Index of the node denoting the longest palindromic proper suffix of
/// the current prefix
void init() {
  str[0] = -1;
  nc = 2;
